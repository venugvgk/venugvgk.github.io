...
The coefficients $a$, $b$, and $c$ play different roles. Understanding those roles is more useful than memorising a collection of graph shapes.
...
- The graph is a parabola, which is symmetric about a vertical line called the **axis of symmetry**. The axis of symmetry is parallel to the $y$-axis. 
- The lowest or highest point of the parabola is called the **vertex**. The axis of symmetry passes through the vertex. The vertex is a minimum point if the parabola opens upwards and a maximum point if it opens downwards.
...
The values for $-x$ and $x$ are equal:
...
## 3. What happens for positive and negative values of $a$?
...
The value of $y$ is minimum when $x=-1$, and that value is $y=-1$,so the vertex is $(-1,-1)$ 

The axis of symmetry is a line passing through $x=-1$.
...
The value of $y$ is minimum when $x=1$, and that value is $y=-1$,so the vertex is $(1,-1)$ and the axis of symmetry is $x=1$.
...
The number of real $x$-intercepts depends on the solutions of this equation, which in turn depends on the discriminant.
...
[GeoGebra Calculator](https://www.geogebra.org/calculator) - Use the graphing suite to plot various linear and quardratic relationships.

[Graph of Quadratic Equations](https://www.geogebra.org/m/vb2qb9eq) - A GeoGebra activity to explore the effect of changing $a$, $b$, and $c$ on the graph of a quadratic equation.

[parabolic patterns](https://nrich.maths.org/problems/parabolic-patterns?tab=overview) - Identify the graphs corresponding to different quadratic equations.
