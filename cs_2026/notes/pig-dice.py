...
def player_turn(score):
    turn_total = 0
    while True:
        print(f"\nYour score: {score} | Turn total: {turn_total}")
...
def computer_turn(score):
    turn_total = 0
    print(f"\nComputer's turn. Current score: {score}")
    while True:
        if score + turn_total >= TARGET_SCORE:
...
def main():
    ...
    while player_score < TARGET_SCORE and computer_score < TARGET_SCORE:
        gained = player_turn(player_score)
        player_score += gained
        print(f"Your total score is now {player_score}.")
        if player_score >= TARGET_SCORE:
            break

        gained = computer_turn(computer_score)
        computer_score += gained
